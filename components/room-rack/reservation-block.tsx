...
import { Ban, CheckCircle2, CreditCard, Crown, Users } from "lucide-react"
...
const INACTIVE_STATUSES = new Set<ReservationStatus>(["cancelled", "no_show"])
...
export function ReservationBlock({
  ...
  const isInactive = INACTIVE_STATUSES.has(reservationStatus)
  const isTerminal = reservationStatus === "checked_out" || isInactive
  ...
  const { attributes, listeners, setNodeRef, isDragging: dndDragging, transform } = useDraggable({
    ...
    disabled: isTerminal || resizing !== null,
  })
  ...
  const translateStyle: React.CSSProperties | undefined = transform
    ? { transform: dndDragging ? undefined : `translate3d(${transform.x}px, ${transform.y}px, 0)`, zIndex: 60 }
    : undefined

  const guestName = `${block.guest.first_name} ${block.guest.last_name}`.trim()
  const hasBalance = block.balance > 0.01
  const showFullDetails = !compact && !isInactive
  const showCompactDetails = compact && !isInactive && width >= 64
  const showCompactStatus = compact && !isInactive && width >= 124
  const showCompactPayment = compact && !isInactive && width >= 170
  const blockTitle = [
    guestName || "Гість не вказаний",
    `№ ${block.reservation_number}`,
    `${block.check_in} - ${block.check_out}`,
    `${block.nights} н.`,
    RESERVATION_STATUS_SHORT_UK[reservationStatus],
    hasBalance ? `Баланс: ${block.balance.toFixed(0)}` : "Оплачено",
  ].join(" · ")
  ...
  return (
    <div
      ...
      className={cn(
        "group absolute cursor-grab select-none overflow-hidden rounded-md border text-xs shadow-sm transition-all",
        isInactive ? "top-auto bottom-1 h-5" : "top-1.5 bottom-1.5",
        bgCls,
        borderCls,
        isTerminal && "cursor-default",
        isInactive && "border-dashed opacity-80 shadow-none saturate-75",
    ...
    >
      ...
      {!isTerminal && block.check_in >= gridStartIso && (
        <div
          role="separator"
      ...
      {!isTerminal && block.check_out <= gridEndIso && (
        <div
          role="separator"
      ...
      {/* контент */}
      <div className={cn("flex h-full flex-col gap-0.5 px-2 py-1 pl-3", compact && "justify-center py-1", isInactive && "py-0 pl-2.5")}>
        <div className="flex items-center gap-1 truncate text-[11px] font-semibold leading-tight">
          {isInactive && <Ban className="h-3 w-3 shrink-0 opacity-75" aria-hidden="true" />}
          {block.guest.is_vip && <Crown className="h-3 w-3 shrink-0 text-amber-500" aria-label="VIP гість" />}
          <span className="truncate">{guestName || "—"}</span>
          {isInactive && (
            <span className="ml-auto shrink-0 rounded bg-background/70 px-1 text-[10px] font-medium">
              {RESERVATION_STATUS_SHORT_UK[reservationStatus]}
            </span>
          )}
        </div>
        ...
        {showCompactDetails && (
          <div className="flex min-w-0 items-center gap-1.5 overflow-hidden whitespace-nowrap text-[10px] leading-tight opacity-85">
            <span className="min-w-0 truncate tabular-nums">№ {block.reservation_number}</span>
            <span className="shrink-0 tabular-nums">{block.nights} н.</span>
            {showCompactStatus && (
              <span className="shrink-0 rounded bg-background/60 px-1 font-medium">
                {RESERVATION_STATUS_SHORT_UK[reservationStatus]}
              </span>
            )}
          </div>
        ...
        {showCompactPayment && (
          <div className="flex items-center gap-1.5 truncate text-[10px] leading-tight">
            {hasBalance ? (
...
export function ReservationBlockPreview({
  block,
  width,
  compact,
}: {
  ...
  const guestName = `${block.guest.first_name} ${block.guest.last_name}`.trim()
  const hasBalance = block.balance > 0.01
  const showFullDetails = !compact
  const showCompactDetails = compact && width >= 64
  const showCompactStatus = compact && width >= 124
  const showCompactPayment = compact && width >= 170

  return (
    <div
      className={cn(
        "pointer-events-none relative h-12 overflow-hidden rounded-md border text-xs shadow-2xl ring-2 ring-primary/20",
        bgCls,
        borderCls,
      )}
      style={{ width: `${Math.max(96, width)}px` }}
    >
      <div className={cn("absolute left-0 top-0 bottom-0 w-1", accent)} />
      <div className={cn("flex h-full flex-col gap-0.5 px-2 py-1 pl-3", compact && "justify-center py-1")}>
        <div className="flex items-center gap-1 truncate text-[11px] font-semibold leading-tight">
          {block.guest.is_vip && <Crown className="h-3 w-3 shrink-0 text-amber-500" aria-label="VIP гість" />}
          <span className="truncate">{guestName || "—"}</span>
        </div>
