...
const HOUSEKEEPING_INSPECTION_SOURCE_TASK_TYPES = new Set([
  "standard_cleaning",
  "deep_cleaning",
  "turndown",
  "checkout_cleaning",
...
/**
 * Чи треба після завершення задачі прибирання автоматично створити окрему
 * перевірку номера.
 */
export function shouldAutoCreateInspection(taskType?: string | null): boolean {
  if (!taskType) return false
  return HOUSEKEEPING_INSPECTION_SOURCE_TASK_TYPES.has(taskType)
}

export function isHousekeepingInspectionTask(taskType?: string | null): boolean {
