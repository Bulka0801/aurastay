 ...
 * За замовчуванням використовується UAH/uk-UA, але критичні PMS-сценарії
 * можуть передати hotel_settings і отримати актуальну валюту/локаль.
 */

import { DEFAULT_HOTEL_SETTINGS, type HotelSettings } from "@/lib/hotel-settings"
...
export function formatMoney(
  ...
  settings?: Pick<HotelSettings, "currency" | "locale"> | null,
  options?: Intl.NumberFormatOptions,
): string {
  const value = Number(amount ?? 0)
  const safeValue = Number.isFinite(value) ? value : 0
  const currency = settings?.currency || DEFAULT_HOTEL_SETTINGS.currency
  const locale = settings?.locale || DEFAULT_HOTEL_SETTINGS.locale
  ...
  try {
    ...
    return uahFmt.format(safeValue)
  }
}
